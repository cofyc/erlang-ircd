...
parse(Line) ->
    parse_words(string:tokens(clean(Line), " ")).

%% compose/1

compose(M = #irc_message{prefix = false}) ->
    compose_message(M);
...
reply('ERR_NOSUCHNICK', Target, [Nick]) ->
    reply_format(401, [Target, Nick],
		 "No such nick/channel");
reply('ERR_NOSUCHSERVER', Target, [ServerName]) ->
    reply_format(402, [Target, ServerName],
		 "No such server");
reply('ERR_NOSUCHCHANNEL', Target, [ChannelName]) ->
    reply_format(403, [Target, ChannelName],
		 "No such channel");
reply('ERR_CANNOTSENDTOCHAN', Target, [ChannelName]) ->
    reply_format(404, [Target, ChannelName],
...
reply('ERR_NOADMININFO', Target, [Server]) ->
    reply_format(423, [Target, Server],
		 "No administrative info available");
reply('ERR_FILEERROR', Target, [FileOp, File]) ->
    reply_format(424, [Target], "File error doing ~s on ~s",
		 [FileOp, File]);
...
reply('ERR_NICKNAMEINUSE', Target, [Nick]) ->
    reply_format(433, [Target, Nick],
		 "Nickname is already in use");
reply('ERR_NICKCOLLISION', Target, [Nick]) ->
    reply_format(436, [Target, Nick],
		 "Nickname collision KILL");
reply('ERR_USERNOTINCHANNEL', Target,
      [Nick, Channel]) ->
    reply_format(441, [Target, Nick, Channel],
		 "They aren't on that channel");
reply('ERR_NOTONCHANNEL', Target, [Channel]) ->
    reply_format(442, [Target, Channel],
		 "You're not on that channel");
reply('ERR_USERONCHANNEL', Target, [User, Channel]) ->
    reply_format(443, [Target, User, Channel],
...
reply('ERR_CHANOPRIVSNEEDED', Target, [Channel]) ->
    reply_format(482, [Target, Channel],
...
reply('ERR_USERSDONTMATCH', Target, []) ->
    reply_format(502, [Target],
		 "Cant change mode for other users");
reply('RPL_USERHOST', Target, Replies) ->
    reply_format(302, [Target], "[~s]",
		 [string:join(Replies, " ")]);
reply('RPL_ISON', Target, Nicks) ->
    reply_format(303, [Target], "[~s]",
		 [string:join(Nicks, " ")]);
reply('RPL_AWAY', Target, [Nick, AwayMessage]) ->
    reply_format(301, [Target, Nick], "~s", [AwayMessage]);
reply('RPL_UNAWAY', Target, []) ->
    reply_format(305, [Target],
		 "You are no longer marked as being away");
reply('RPL_NOWAWAY', Target, []) ->
    reply_format(306, [Target],
		 "You have been marked as being away");
reply('RPL_WHOISUSER', Target,
      [Nick, User, Host, RealName]) ->
    reply_format(311, [Target, Nick, User, Host, "*"], "~s",
		 [RealName]);
reply('RPL_WHOISSERVER', Target,
      [Nick, Server, ServerInfo]) ->
    reply_format(312, [Target, Nick, Server], "~s",
		 [ServerInfo]);
...
reply('RPL_WHOISIDLE', Target, [Nick, Integer]) ->
    reply_format(317, [Target, Nick, Integer],
		 "seconds idle");
...
reply('RPL_WHOISCHANNELS', Target, [Nick, Channels]) ->
    reply_format(319, [Target, Nick], "~s",
		 [string:join(Channels, " ")]);
reply('RPL_WHOWASUSER', Target,
      [Nick, User, Host, RealName]) ->
    reply_format(314, [Target, Nick, User, Host, "*"], "~s",
		 [RealName]);
...
reply('RPL_LIST', Target,
      [Channel, NumVisible, Topic]) ->
    reply_format(322, [Target, Channel, NumVisible], "~s",
		 [Topic]);
reply('RPL_LISTEND', Target, []) ->
    reply_format(323, [Target], "End of /LIST");
reply('RPL_CHANNELMODEIS', Target,
      [Channel, Mode, ModeParams]) ->
    reply_format(324, [Target, Channel, Mode, ModeParams],
		 false);
...
reply('RPL_SUMMONING', Target, [User]) ->
    reply_format(342, [Target, User],
		 "Summoning user to IRC");
reply('RPL_VERSION', Target,
      [Version, DebugLevel, Server, Comments]) ->
    reply_format(351,
		 [Target, Version ++ "." ++ DebugLevel, Server], "~s",
		 [Comments]);
reply('RPL_WHOREPLY', Target,
      [Channel, User, Host, Server, Nick, Flags, HopCount,
       RealName]) ->
    reply_format(352,
		 [Target, Channel, User, Host, Server, Nick, Flags],
...
reply('RPL_ENDOFNAMES', Target, [Channel]) ->
    reply_format(366, [Target, Channel],
		 "End of /NAMES list");
reply('RPL_LINKS', Target,
      [Mask, Server, HopCount, ServerInfo]) ->
    reply_format(364, [Target, Mask, Server], "~b ~s",
		 [HopCount, ServerInfo]);
...
reply('RPL_ENDOFBANLIST', Target, [Channel]) ->
    reply_format(368, [Target, Channel],
...
reply('RPL_MOTDSTART', Target, [Server]) ->
    reply_format(375, [Target],
		 "- ~s Message of the day - ", [Server]);
...
compose_message(#irc_message{command = Command,
			     params = Params, trailing = Trailing}) ->
    [stringify(Command),
     case Params of
       [] -> "";
       _ ->
	   [" ", string:join([stringify(P) || P <- Params], " ")]
     end,
     case Trailing of
       false -> "";
       _ -> [" :", Trailing]
...
parse_words([Command | Rest]) ->
    ...
    #irc_message{prefix = false, command = Command,
		 params = Params, trailing = Trailing}.

parse_params([":" ++ FirstTrailing | Rest],
	     ParamsRev) ->
    {lists:reverse(ParamsRev),
     string:join([FirstTrailing | Rest], " ")};
